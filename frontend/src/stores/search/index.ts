...
import type { Repository, SearchQuery } from '@/stores/search/types'
...
export const useSearchStore = defineStore('search', {
  ...
  actions: {
    async packageAutocomplete(payload: AutocompletePayload) {
      this.packageOptions = await apiRegistry
        .get()
        .httpReq(this.actionRequests.packageAutocomplete, { data: payload })
    },
    resetPackageOptions() {
      this.packageOptions = []
    },
    async search(search: SearchQuery) {
      const responseJson = await apiRegistry.get().httpReq<{
        total: number
        results: Repository[]
      }>(this.actionRequests.search, { data: search })
