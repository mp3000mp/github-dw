...
describe('utils/version.ts', () => {
  ...
  it.each<string | null>([null, '', '0.1', '1', '1.', '1.2', '1.2.', '1.2.3'])(
    'function validVersion truthy with input %s',
    (input) => {
      expect(validVersion(input)).toBeTruthy()
    }
  )
  it.each<string>(['0', '.1', 'a', '1.a'])('function validVersion falsy with input %s', (input) => {
