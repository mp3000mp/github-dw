...
import type { ChartDataset, ChartItem, TooltipItem } from 'chart.js/auto'
import { LanguageColorEnum } from '@/stores/search/types'
import type { Routine1Timeline } from '@/stores/admin/types'

const adminStore = useAdminStore()

const camembertRef = ref(null) as Ref<ChartItem | null>
let camembertGraph: Chart<'pie'> | null = null
const timelineRef = ref(null) as Ref<ChartItem | null>
let timelineGraph: Chart<'bar' | 'line'> | null = null
        ...
        tooltip: {
          callbacks: {
            label: function (ctx: TooltipItem<'pie'>) {
              ...
              return ctx.label + ': ' + ctx.formattedValue + ' (' + percent(ctx.parsed, total) + ')'
            }
          }
        }
      }
...
watch(timeline, () => {
  ...
  timelineGraph = new Chart<'bar' | 'line', number[]>(timelineRef.value, {
    type: 'bar',
    data,
    options: {
      interaction: {
  ...
  <div class="container">
    ...
    <div v-else>
      <div class="row app-block mb-2 p-3">
        <h2 class="mb-2">
          Package file types
          <span><font-awesome class="cp" icon="refresh" @click="refresh" /></span>
        </h2>
        <div class="col-md-8">
          <table>
            <thead>
              ...
              <tr v-for="packageTypeFile in tableData" :key="packageTypeFile.id">
                ...
                <td>
                  ...
                  <tooltip-popper v-else content="Set priority" :hover="true" :arrow="true">
                    <font-awesome
                      class="cp"
                      @click="setPriority(packageTypeFile.id)"
                      icon="ellipsis"
                    />
                  </tooltip-popper>
                </td>
      ...
      <div class="row app-block p-3">
        ...
        <ul class="col">
          <li class="mx-1" v-for="error in errors.routine2" :key="error.date">
            {{ dayjs(error.date).format('YYYY-MM-DD HH:mm:ss') }} - {{ error.error }} ({{
              error.url
            }})
          </li>
        </ul>
