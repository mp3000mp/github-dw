...
const autocompleteOptions = computed(() =>
  autocompleteResults.value.map((ar) => ({ value: ar.id, label: ar.name }))
)
const isAlreadyInSearch = computed(
  () => dependencies.value.findIndex((d) => d.id === selectedDependencyId.value) >= 0
)
  ...
  <div class="container">
    <form class="row">
      ...
      <div
        ...
        v-if="shownPackages.length > 0 || searchRequest.callCount > 0"
      >
        <div class="row">
          <h2>Selected dependencies</h2>
          <div v-if="shownPackages.length === 0">None</div>
