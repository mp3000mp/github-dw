...
<template>
  <div class="card">
    <div class="card-header p-3">
      <div class="row">
        <h3 class="col-md-6 mb-2 mb-md-0">
          <a :href="repository.url" target="_blank">{{ repository.fullName }}</a>
        </h3>
        <div class="col-md-6 text-md-end">
          ...
          <span class="ml-2">
            <tooltip-popper content="Last pushed at" :hover="true" :arrow="true">
              <span>
                <font-awesome icon="clock-rotate-left" />
                {{ dayjs(repository.pushedAt).format('YYYY-MM-DD') }}
              </span>
            </tooltip-popper>
          </span>
        </div>
    ...
    <div class="card-body p-3 fs-09">
      ...
      <div v-if="repository.topics.length > 0">
        Topics:
        <span class="badge" v-for="topic in repository.topics" :key="topic.topic">{{
          topic.topic
        }}</span>
      </div>
    </div>
